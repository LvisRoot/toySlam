...
MatrixXd computeJack(const std::vector<Vector2d> & t_points, const std::vector<frame> & t_frames){
	...
	for(int j = 0; j < mT ; ++j){
		for(int i = 0; i < nP ; ++i){
			...
			jack.block<1,2>(nP*j+i,2*j) = computePoseJack(t_points[i],t_frames[j]._pose).transpose();

			jack.block<1,2>(nP*j+i,2*mT+2*i) = computePointJack(t_points[i],t_frames[j]._pose).transpose();
		}
	}
	return jack;
}
...
//	}
	...
	points.push_back(Vector2d(3,1));
	points.push_back(Vector2d(5,2));
	points.push_back(Vector2d(8,3));

	poses.push_back(Vector2d(2,-1));
	poses.push_back(Vector2d(4,-1));
	poses.push_back(Vector2d(6,-2));
	poses.push_back(Vector2d(7,-3));
	...
	for (int j = 0; j < mT; ++j) {
		...
		for (int i = 0; i < nP; ++i){
			meas.push_back(project(points[i],poses[j]));

			std::cout << "\n Point: " << i << " Proj. in Pose: " << j << '\n' << project(points[i],poses[j]) << '\n';
		}
		frames.push_back(frame{poses[j],meas});
	}
